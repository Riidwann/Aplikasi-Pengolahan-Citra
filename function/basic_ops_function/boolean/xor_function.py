...
def boolean_xor(image_label, image_result_label, result_text_label):
  ...
  try:
    ...
    # Eksekusi operasi Boolean XOR (Difference)
    result_img = ImageChops.difference(img_a_rgb, img_b_rgb)
    ...
    result_text_label.config(text="Hasil Operasi XOR")

  except Exception as e:
      # Penanganan eksepsi proses
      messagebox.showerror("Error", f"Gagal melakukan XOR: {e}")